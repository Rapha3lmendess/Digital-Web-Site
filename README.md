...
`server/index.js` verifica o bearer token do Supabase Auth. Rotas: `GET /api/profile`, `GET/POST /api/payments`, `POST /api/downloads` e `POST /api/account/delete`. Consultas de perfil/pedido/download usam o JWT do usuário e mantêm as políticas RLS; a atualização do nome usa Supabase Auth no cliente. A chave administrativa é opcional e só é usada para exclusão da identidade autenticada.
...
- **Vercel:** há configuração para publicar site estático e API no mesmo projeto; consulte [`Docs/deploy-vercel.md`](Docs/deploy-vercel.md). A chave `SUPABASE_SERVICE_ROLE_KEY` é opcional para iniciar a API e só é necessária para exclusão de conta.
...
4. Execute `npm install` com Node.js 22.x e copie `.env.example` para `.env`. Preencha as variáveis Supabase e `ALLOWED_ORIGINS`; o exemplo já inclui localhost e a origem do GitHub Pages. `SUPABASE_SERVICE_ROLE_KEY` só fica no servidor.
5. Inicie a API Node.js com `npm start`. Ela valida o token do usuário; exclusão de conta exige `SUPABASE_SERVICE_ROLE_KEY` no servidor e só permite excluir a própria conta. Se frontend e API forem hospedados em origens diferentes, configure `API_BASE_URL` em `js/supabase-config.js` e inclua a origem em `ALLOWED_ORIGINS`.
